...
class Member extends Base {
    ...
    */
     ...
     */
    public function ajax_batch_update_userslevel()
    {
        function_exists('set_time_limit') && set_time_limit(10);
        \think\Session::pause(); // 暂停session，防止session阻塞机制
        $redata = $this->batch_update_userslevel();
     ...
     * @return [type] [description]
     */
    private function batch_update_userslevel()
    {
        $redata = [
            'code' => 1,
            'msg'  => '没开启会员中心，不需要执行',
        ];
        $web_users_switch = $this->globalConfig['web_users_switch'];
        if (!empty($web_users_switch)) {
            ...
            $count = Db::name('users')->where($where_str)->fetchSql(true)->count();
            if (!empty($count)) {
